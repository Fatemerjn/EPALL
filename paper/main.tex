...
\begin{table}[t]
\centering
\caption{Standard Split-CIFAR-100 protocol (20 epochs/task), all methods. \(A_{final}\pm\)std over two seeds; other columns are seed means. Random-guess Au \(=0.2\).}
...
LoRA & 0.1000 \(\pm\) 0.0000 & 0.0000 & 0.0000 & 0.0000 & 0.1000 & 7.7062 \\
...
% TODO(data): cross-architecture ViT-T/8 results (tags cifar10_vit_v1 / cifar100_vit_v1)
% are NOT yet in results/aggregates/server_report_table.csv, so the cross-architecture
% WorstDrop-reduction claim is omitted. Add a row/sentence once g11_vit has been run.
% TODO(data): anchor ablation (tag anchor_ablation_v1, old vs reinit L2 anchor) and the
% agreement-rate / gradient-norm-ratio diagnostics (tag agreement_v1) are not yet in the
% aggregates; add them when populated.
