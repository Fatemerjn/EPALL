...
By quarantining weight updates strictly to the bottleneck modules, the PALL-Adapter sharply reduces the updated-parameter ratio (\(R_{updated}\)) relative to the full-network PALL-Original on every benchmark: from \(0.0475\) to \(0.0061\) on CIFAR-10 (a \(\sim\!87\%\) reduction), from \(0.0763\) to \(0.0123\) on CIFAR-100 (\(\sim\!84\%\)), and from \(0.0960\) to \(0.0292\) on TinyImageNet (\(\sim\!70\%\)). Figure \ref{fig:worstdrop_chart} confirms that this parameter efficiency directly prevents WorstDrop degradation.
