...
PALL-Modified formulates the unlearning process as a constrained optimization problem. While shifting the output of the forgotten dataset to a uniform distribution, a regularization penalty \(\lambda\) is enforced exclusively on the critical intersection:
\begin{equation}
\mathcal{L}_{modified} = \mathcal{L}_{unlearn} + \lambda \sum_{w \in S_{share\_crit}} \left\| w - w_{old} \right\|^2
\end{equation}
This regularization actively prevents the algorithm from destroying the shared functional space, yielding the highest retention accuracy, albeit operating over the entire network capacity.
...
\end{itemize}
All algorithms were trained using a ResNet-18 backbone \cite{he2016deep} via Stochastic Gradient Descent (SGD) \cite{robbins1951sgd} with a learning rate of \(0.01\), over 3 epochs per task. Baseline comparisons include ER \cite{thrun1995lifelong} and DER++ \cite{buzzega2020dark}.

\section{Experimental Results and Analysis}

\begin{table*}[t]
\centering
\caption{Comprehensive Quantitative Evaluation of Proposed Methods vs. Baselines}
\label{tab:main_results}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llcccccc@{}}
\toprule
\textbf{Dataset} & \textbf{Algorithm} & \textbf{\(A_{final}\) (\(\uparrow\))} & \textbf{\(F_{avg}\) (\(\downarrow\))} & \textbf{WorstDrop (\(\downarrow\))} & \textbf{Au (\(\downarrow\))} & \textbf{Fu} & \textbf{\(R_{updated}\) (\(\downarrow\))} \\ \midrule
\multirow{5}{*}{\textbf{CIFAR-10}} 
& DER++ & 0.9244 \(\pm\) 0.0167 & 0.2399 & 0.0158 & 0.2678 & 0.0061 & N/A \\
& ER & 0.8864 \(\pm\) 0.0277 & 0.2278 & 0.0557 & 0.5318 & 0.0307 & N/A \\
& PALL-Original & 0.9166 \(\pm\) 0.0209 & 0.1583 & 0.0160 & 0.5000 & 0.0056 & 0.0475 \\
& \textbf{PALL-Modified (Ours)} & \textbf{0.9301 \(\pm\) 0.0094} & 0.1507 & \textbf{0.0008} & 0.5005 & 0.0004 & 0.0475 \\
& \textbf{PALL-Adapter (Ours)} & 0.8057 \(\pm\) 0.0054 & \textbf{0.0916} & 0.0050 & \textbf{0.5000} & -0.0369 & \textbf{0.0061} \\ \midrule
\multirow{5}{*}{\textbf{CIFAR-100}} 
& DER++ & 0.5371 \(\pm\) 0.0097 & 0.1040 & 0.0440 & 0.3590 & 0.0213 & N/A \\
& ER & 0.4657 \(\pm\) 0.0675 & 0.1854 & 0.1400 & 0.1870 & 0.0734 & N/A \\
& PALL-Original & 0.4233 \(\pm\) 0.0297 & 0.1611 & 0.0710 & 0.2250 & 0.0294 & 0.0763 \\
& \textbf{PALL-Modified (Ours)} & \textbf{0.5377 \(\pm\) 0.0299} & 0.1045 & 0.0140 & 0.2120 & -0.0071 & 0.0763 \\
& \textbf{PALL-Adapter (Ours)} & 0.4084 \(\pm\) 0.0293 & \textbf{0.0853} & \textbf{0.0050} & \textbf{0.1640} & 0.0020 & \textbf{0.0123} \\ \midrule
\multirow{3}{*}{\textbf{TinyImageNet}} 
& PALL-Original & 0.3361 \(\pm\) 0.0141 & 0.1949 & 0.0940 & 0.0960 & 0.0534 & 0.0960 \\
& \textbf{PALL-Modified (Ours)} & \textbf{0.4566 \(\pm\) 0.0054} & 0.0869 & 0.0600 & 0.0960 & 0.0157 & 0.0960 \\
& \textbf{PALL-Adapter (Ours)} & 0.3256 \(\pm\) 0.0064 & \textbf{0.0758} & \textbf{0.0080} & \textbf{0.0800} & 0.0057 & \textbf{0.0292} \\ \bottomrule
\end{tabular}%
}
\end{table*}

\subsection{Accuracy vs. Forgetting Stability Trade-off}
As shown in Table \ref{tab:main_results}, \textbf{PALL-Modified} achieves the highest retention accuracy among the compared methods (\(A_{final} = 0.9301\) on CIFAR-10) by balancing explicit overlap regularization with full-capacity network training. Conversely, \textbf{PALL-Adapter} prioritizes structural stability and parameter efficiency. On CIFAR-100, the PALL-Adapter restricts the WorstDrop to an exceptional \(0.0050\), compared to a catastrophic \(0.0710\) drop in PALL-Original, proving that targeted soft-scaling prevents interference.

\subsection{Computational Efficiency and Ablation Study}
On TinyImageNet, traditional unlearning recalculates \(9.6\%\) of the entire deep network's parameters. The PALL-Adapter quarantines weight updates strictly to the bottleneck modules, requiring only \(2.92\%\) of parameters to be adjusted, establishing an 87\% relative reduction on CIFAR-10. Figure \ref{fig:worstdrop_chart} confirms that this parameter efficiency directly prevents WorstDrop degradation.
...
\STATE Compute absolute gradients \(|\nabla \mathcal{L}|\) on replay buffer to extract \(S_{forget}\)
\STATE Compute absolute gradients \(|\nabla \mathcal{L}|\) on replay buffer to extract \(S_{active}\)
\STATE \(S_{share\_crit} \leftarrow S_{forget} \cap S_{active}\)
\vspace{1.5mm}
...
\FOR{iteration \(= 1, 2, \dots, N_{iters}\)}
    ...
    \FORALL{weight \(w \in \phi_{shared}\)}
        \IF{\(w \in \text{top}(\alpha_p, S_{active})\)}
            \STATE \(m_{soft} \leftarrow (1.0 - \text{protect\_strength})\) \COMMENT{Dampened friction}
            \STATE \(w \leftarrow w - \eta \cdot m_{soft} \cdot \nabla_{w}\)
        \ELSIF{\(w \in \text{top}(\alpha_f, S_{forget})\)}
            \STATE \(w \leftarrow w - \eta \cdot \nabla_{w}\) \COMMENT{Surgically Scrubbed (Full Update)}
        \ELSE
            \STATE \(w \leftarrow w\) \COMMENT{Untouched parameters}
        \ENDIF
    \ENDFOR
\ENDFOR
\end{algorithmic}
