# سامانه بصری و دستور دقیق شکل‌های پایان‌نامه

این سند پس از بررسی نسخه کامل پایان‌نامه، قالب LaTeX پروژه، ویرایش هشتم دستورالعمل
تهیه پایان‌نامه دانشگاه صنعتی شریف و راهنمای نگارش و ماشین‌نویسی تهیه شده است.

## تم منتخب: «مینیمال فنی شریف»

- زمینه: سفید خالص، بدون بافت، سایه، شیب و جلوه سه‌بعدی.
- نسبت شکل‌های مفهومی: افقی و کم‌ارتفاع، ترجیحا `16:6` یا `16:7`.
- قلم نوشته‌های لاتین و ریاضی: `Times New Roman` با جایگزین `DejaVu Serif`.
- حداقل اندازه نوشته پس از درج در صفحه A4: ۹ پوینت.
- ضخامت خط اصلی: ۱ تا ۱٫۴ پوینت در اندازه نهایی؛ خطوط فرعی ۰٫۶ تا ۰٫۸ پوینت.
...
- متن داخل شکل کوتاه، فنی و ترجیحا انگلیسی/نماد ریاضی است؛ توضیح بلند در کپشن می‌آید.
...
| تأکید ثانویه ناحیه بحرانی | `#6F3C8F` |
| اجزای منجمد و خنثی | `#69757F` |
| متن اصلی | `#263238` |

## قواعد مشترک همه دستورها

عبارت زیر باید به انتهای هر دستور تصویری افزوده شود:
...
برای نمودارهای تجربی استفاده از مدل تولید تصویر ممنوع است. دستور آن‌ها برای یک عامل
...
**وضعیت:** جایگزین شود. نسخه فعلی عمودی است و فضای A4 را هدر می‌دهد.
...
### ۳. شکل تازه فصل مرور ادبیات

**محل:** بلافاصله پیش از بخش «مقایسه مفهومی و جایگاه این پژوهش».
...
**وضعیت:** جایگزین شود؛ نسخه فعلی عمودی است.
...
**وضعیت:** جایگزین شود؛ شکل فعلی بلند است و نسخه تازه ChatGPT نیز انقباض را درست
نمایش نمی‌دهد.
...
همان دستور شکل ۹، با `CIFAR-100` به‌عنوان تحلیل اصلی و بدون تغییر در مشاهدات.
...
همان دستور شکل ۱۱، با محور عمودی `Signed WorstDrop`، خط صفر و حفظ مقادیر منفی.
...
## نه تصویر تازه پیوست‌شده

### تصویر تازه ۱ — Shared-bottleneck ablation

**حکم:** معتبر و قابل استفاده؛ همان دستور شکل ۱۷. فقط تمام‌عرض درج شود، نه کنار یک
نمودار عمودی.

### تصویر تازه ۲ — Subphase drop-budget decomposition

**حکم:** از نظر ایده مناسب، ولی نسبت انقباض غلط است. از دستور شکل ۶ استفاده شود.

### تصویر تازه ۳ — معماری افقی PALL-Adapter

**حکم:** پس از اصلاح فنی مناسب است. از دستور شکل ۵ استفاده شود؛ به‌طور ویژه ورودی
`B×3×H×W` و طبقه‌بند `C×512` با بلوک سطرهای کلاس وظیفه حفظ شود.

### تصاویر تازه ۴، ۵، ۶ و ۷ — نمودارهای روش/دیتاست مقایسه‌ای
...
### تصویر تازه ۸ — شبکه هم‌پوشانی و مجموعه بحرانی

**حکم:** فقط پس از بازسازی قطعی. از دستور شکل ۲ استفاده شود؛ مختصات شبکه در هر سه
پنل باید دقیقا یکسان باشند.

### تصویر تازه ۹ — خط زمانی یادگیری و حذف

**حکم:** مجموعه‌های فعال نسخه پیوست غلط‌اند. از دستور شکل ۱ استفاده شود. نسخه پایان‌نامه
