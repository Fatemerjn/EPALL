#!/usr/bin/env python3
"""
Generate or run pall_adapter experiments over candidate overlap schedules.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Generate or run pall_adapter candidate-overlap search commands.")
...
def build_commands(args: argparse.Namespace, repo_root: Path, rows: Sequence[dict[str, str]]) -> list[tuple[int, str, str, list[str]]]:
    preset = DATASET_PRESETS[args.dataset]
    main_file = Path(args.main_file).expanduser().resolve()
    ...
    for row in rows:
        ...
        for method in args.methods:
            ...
            commands.append((forget_task_id, method, schedule_rel, command))
    return commands


def print_command(index: int, total: int, forget_task_id: int, method: str, schedule_path: str, command: Sequence[str]) -> None:
    print(f"[{index}/{total}] forget_task_id={forget_task_id} method={method} schedule={schedule_path}")
    print(f"  {shlex.join(list(command))}")


def run_commands(commands: Sequence[tuple[int, str, str, list[str]]], cwd: Path, execute: bool) -> int:
    total = len(commands)
    for index, (forget_task_id, method, schedule_path, command) in enumerate(commands, start=1):
        print_command(
            index=index,
            total=total,
